...
* Executing ping tests from Router1 demonstrates successful connectivity to `10.0.1.1` (R3's Ethernet0/1). However, pings to `10.0.22.2` (R3's Ethernet0/2) fail completely.
* A quick check of Router1's routing table reveals the root cause for the ping drop: R1 does not have a route to the `10.0.22.0` network.
  ![Reachability Test](./8.png)
